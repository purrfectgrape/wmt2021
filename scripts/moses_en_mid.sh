...
while getopts ":c:" opt; do
  case $opt in
    c)
      ...
      cat $BASE/data/raw/$OPTARG-langid-filtered.en | $BASE/libraries/moses/scripts/tokenizer/normalize-punctuation.perl -l en > $BASE/data/preprocessed/$OPTARG-punc-normalized.en
...
for c in wikimatrix paracrawl newscommentary reuters newsdev2020; do
    ...
    wc -l $BASE/data/raw/$c-langid-filtered.ja
done
echo "##############################################"
