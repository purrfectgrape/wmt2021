...
for checkpoint in $BASE/models/hiragana_4m/hiragana_4m_step*.pt; do
    ...
    $HOME/.local/bin/onmt_translate \
        ...
        -beam_size 5 \
        -model $checkpoint \
        -src $BASE/data/test/preprocessed/newstest2020-jaen-hiragana.sp.ja \
        -tgt $BASE/data/test/preprocessed/newstest2020-jaen-hiragana.sp.en \
...
for checkpoint in $BASE/models/hiragana_4m/hiragana_4m_step*.pt; do
    base=$(basename $checkpoint)
    spm_decode \
        -model=$BASE/sentencepiece/hiragana_sample_4m.en.model \
        -input_format=piece \
